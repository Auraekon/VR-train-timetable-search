                ...
                <TableRow key={n.trainNumber} style={{backgroundColor: rowShadeColor}}>
                  ...
                  <TableCell cellContent={   
                    n.timeTableRows.map(m => {
                      if (m.stationShortCode === props.currentSearchedStation.stationShortCode && m.type === "ARRIVAL" && props.queryType === "fromStation") {
                        if (m.cancelled == true) {
                          return "cancelled";
                        } else {
                      ...
                      if (m.stationShortCode === props.currentSearchedStation.stationShortCode && m.type === "DEPARTURE" && props.queryType === "toStation") {
                        if (m.cancelled == true) {
                          return "cancelled";
                        } else {
                          let tempTimeStamp = (m.liveEstimateTime ? m.liveEstimateTime : m.scheduledTime);
    ...
    return (
      ...
      <div>
        <Tabs>
          ...
          <TabPanel>
            <TrainTable tableCols={tableCols.arriving} data={trainTimeTableData.fromStation} stationMetadata={stationMetadata} currentSearchedStation={currentSearchedStation} queryType={"fromStation"}/>
          </TabPanel>
          <TabPanel>
            <TrainTable tableCols={tableCols.departing} data={trainTimeTableData.toStation} stationMetadata={stationMetadata} currentSearchedStation={currentSearchedStation} queryType={"toStation"}/>
          </TabPanel>
